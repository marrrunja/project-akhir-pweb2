...
<style>
    ...
    .product-card { border-radius: 15px; overflow: hidden; height: 100%; }
    .category-btn { border-radius: 20px; }
    .category-btn.active { background-color: #ff7f50; color: #fff; }
    .product-card img { height: 250px; width: 100%; object-fit: cover; aspect-ratio: 1 / 1; }
    .product-card .card-body { padding: 1rem; height: 100%; display: flex; flex-direction: column; justify-content: space-between; }
</style>
@endpush
            ...
            <div class="card product-card h-100">
                ...
                <div class="card-body text-center">
                    ...
                    <ul class="list-unstyled">
                        ...
                        <li>{{ $variant->variant }} - Rp{{ number_format($variant->harga ?? 0, 0, ',', '.') }} - Stok: {{ $variant->stok ? $variant->stok->count() : 0 }}</li>
                        @endforeach
                    </ul>
                    ...
                    <form action="{{ route('cart.store') }}" method="POST">
                        ...
                        <div class="modal-body">
                            ...
                            <select class="form-select" name="variant_id" required>
                                @foreach($produk->variant as $variant)
                                <option value="{{ $variant->id }}">{{ $variant->variant }} - Rp{{ number_format($variant->harga ?? 0, 0, ',', '.') }} - Sisa: {{ $variant->stok ? $variant->stok->count() : 0 }}</option>
                                @endforeach
                            </select>
                            <div class="mt-3">
                                <label>Jumlah:</label>
